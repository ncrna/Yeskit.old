...
#' @export
#'
scDensityPlot <- function(object=NULL, reduction=NULL, title=NULL, split.by=NULL, ncol=NULL, colors=NULL, digits=NULL){
    ...
    ps <- function(data, z.min=z.min, z.max=z.max, title=NULL){
      ...
      p <- p + ggplot2::stat_density2d(geom = 'raster', interpolate=TRUE, ggplot2::aes(fill = ..density..), contour = FALSE, na.rm = TRUE) +
               ggplot2::scale_fill_gradientn(colors=colors, limits = c(z.min, z.max), breaks=seq(z.min, z.max, length=5), labels=round(seq(z.min, z.max, length=5), digits=digits)) +
               cowplot::theme_cowplot() + ggplot2::guides(colour=ggplot2::guide_legend(override.aes=list(size=5))) +
