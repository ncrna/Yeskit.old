...
#' @param digits Integer representing the number of decimal places reserved for the legend, default digits=3
#'
#'
...
#' @export
#'
scDensityPlot <- function(object = NULL, reduction = NULL, title = NULL, split.by = NULL,
    ncol = NULL, colors = NULL, digits = NULL) {
    ...
    ps <- function(data, z.min = z.min, z.max = z.max, title = NULL) {
        p <- ggplot2::ggplot(data = data, ggplot2::aes(x = data[, 1], y = data[,
            2]))
        p <- p + ggplot2::stat_density2d(geom = "raster", interpolate = TRUE, ggplot2::aes(fill = ..density..),
            contour = FALSE, na.rm = TRUE) + ggplot2::scale_fill_gradientn(colors = colors,
            limits = c(z.min, z.max), breaks = seq(z.min, z.max, length = 5), labels = round(seq(z.min,
                z.max, length = 5), digits = digits)) + cowplot::theme_cowplot() +
    ...
    m <- MASS::kde2d(x = Data[, 1], y = Data[, 2], h = c(MASS::bandwidth.nrd(Data[,
        1]), MASS::bandwidth.nrd(Data[, 2])), n = 100, lims = c(range(Data[, 1]),
        range(Data[, 2])))
    z.min <- min(m$z)
    z.max <- max(m$z)
    xmin <- min(Data[, 1])
    ...
    if (is.null(x = split.by)) {
        return(ps(Data, z.min, z.max, title))
    }
    plots <- list()
    z <- vector()
    if (!is.null(x = split.by)) {
        ...
        for (s in unique(Data[, split.by])) {
            data <- Data[Data[, 3] == s, ]
            m <- MASS::kde2d(x = data[, 1], y = data[, 2], h = c(MASS::bandwidth.nrd(data[,
                1]), MASS::bandwidth.nrd(data[, 2])), n = 100, lims = c(range(data[,
                1]), range(data[, 2])))
            z <- c(z, range(m$z))
        }
        z <- sort(z)
