...
#' @param category String of specific MSigDB category to use, default category='H' stands for 'hallmark gene sets'
#' @param geneSets Vector of gene sets, default geneSets='NULL' stands for all gene sets in category will be used
...
scMsigdbScoring <- function(object = NULL, category = NULL, geneSets = NULL) {
    ...
    MSIGDB <- read.table(MSigDB, row.names = 1, col.names = paste0("V", seq_len(max(count.fields(MSigDB,
        sep = "\t")))), header = FALSE, sep = "\t", stringsAsFactors = FALSE, fill = TRUE)
    ...
    MSIGDB[, 1] <- NULL
    MSIGDB <- as.list(data.frame(apply(MSIGDB, 1, as.vector), stringsAsFactors = FALSE),
        drop = TRUE)
    ...
    for (item in geneSets) {
        ...
        object.hallmark <- Seurat::AddModuleScore(object = object, features = features,
            name = item, ctrl = min(vapply(X = features, FUN = length, FUN.VALUE = numeric(length = 1))))
        hallmark.columns <- grep(pattern = item, x = colnames(x = object.hallmark[[]]),
            value = TRUE)
