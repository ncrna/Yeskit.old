...
scPopulationPlot <- function(object = NULL, by = c("sample", "cluster"), order = NULL,
    cols = NULL) {
    if (is.null(by)) {
        by = "sample"
    }
    ...
    if (is.null(cols)) {
        ...
        if (length(levels(Seurat::Idents(object))) <= 36) {
            ...
            warning("Not enough colours provided for ", length(levels(Seurat::Idents(object))),
                " clusters! Use ggplot2's default colors instead\n")
            cols <- (scales::hue_pal())(length(levels(Seurat::Idents(object))))
        }
    } else if (length(levels(Seurat::Idents(object))) > length(cols)) {
        stop("Not enough colours provided for ", length(levels(Seurat::Idents(object))),
            " clusters!")
    }
    ...
    if (by == "sample") {
        p <- ggplot2::ggplot(data = data, ggplot2::aes(x = Sample, y = Cells, fill = Cluster)) +
            ggplot2::geom_bar(stat = "identity", position = ggplot2::position_fill(reverse = FALSE),
                width = 0.6, size = 0.3, colour = NA) + ggplot2::xlab("Sample") +
            ggplot2::ylab("Fraction of cells (%)") + cowplot::theme_cowplot() + ggplot2::theme(axis.text.x = ggplot2::element_text(size = 10,
            angle = 45, vjust = 1, hjust = 1), axis.title.x = ggplot2::element_blank(),
            axis.text.y = ggplot2::element_text(size = 10), axis.title.y = ggplot2::element_text(size = 12),
            panel.grid.minor = ggplot2::element_blank()) + ggplot2::scale_fill_manual(values = cols,
            guide = ggplot2::guide_legend(keywidth = 0.5, keyheight = 0.5, ncol = 1)) +
            ggplot2::scale_y_continuous(expand = c(0.01, 0.01), labels = c(0, 25,
                50, 75, 100))
    } else if (by == "cluster") {
        p <- ggplot2::ggplot(data = data, ggplot2::aes(x = Cluster, y = Cells, fill = Sample)) +
            ggplot2::geom_bar(stat = "identity", position = ggplot2::position_fill(reverse = FALSE),
                width = 0.6, size = 0.3, colour = NA) + ggplot2::xlab("Cluster") +
            ggplot2::ylab("Fraction of cells (%)") + cowplot::theme_cowplot() + ggplot2::theme(axis.text.x = ggplot2::element_text(size = 10,
            angle = 45, vjust = 1, hjust = 1), axis.title.x = ggplot2::element_blank(),
            axis.text.y = ggplot2::element_text(size = 10), axis.title.y = ggplot2::element_text(size = 12),
            panel.grid.minor = ggplot2::element_blank()) + ggplot2::scale_fill_manual(values = cols,
