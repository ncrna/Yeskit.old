...
scDimPlot <- function(object = NULL, cols = NULL, pt.size = NULL, reduction = NULL,
    split.by = NULL, label = TRUE, title = NULL, ncol = NULL, raster = TRUE) {
    if (is.null(cols)) {
    ...
    pm <- function(data, title = NULL, legend_title = NULL) {
        p <- ggplot2::ggplot(data = data, ggplot2::aes(x = data[, 1], y = data[,
            2], z = data[, "color"]))
        if (isTRUE(raster)) {
            p <- p + ggrastr::rasterise(dpi = 300, ggplot2::geom_point(colour = data[,
                "color"], size = pt.size))
        } else {
    ...
    object <- Seurat::AddMetaData(object = object, metadata = as.vector(Seurat::Idents(object)),
        col.name = "cluster")
