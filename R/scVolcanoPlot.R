...
#' @param cols Colours to pain the dots, default cols=c("#808080", "#810F7C", "#006D2C", "#000000")
...
scVolcanoPlot <- function(object = NULL, key = NULL, cluster = NULL, padj=0.05, logFC=0.585, cols=NULL, pt.size=NULL,
                         species.by = NULL, top_n = NULL, labels = NULL, label.size = NULL, box = TRUE, raster=TRUE){
  ...
  index <- c()
  if (is.null(labels)){
    labels <- c(head(Data[Data$group=="up", ]$gene, top_n), head(Data[Data$group=="down", ]$gene, top_n))
  }
  ...
  if (isTRUE(raster)){
    p <- p + ggrastr::rasterise(dpi = 300, ggplot2::geom_point(ggplot2::aes(fill=group), size = pt.size, shape = 21, alpha = 0.6, show.legend = FALSE))
    p <- p + ggrastr::rasterise(dpi = 300, ggplot2::geom_point(ggplot2::aes(colour = color), size = pt.size, shape = 21, alpha = 0.6, show.legend = FALSE))
  }else{
    p <- p + ggplot2::geom_point(ggplot2::aes(fill=group), size = pt.size, shape = 21, alpha = 0.6, show.legend = FALSE)
    p <- p + ggplot2::geom_point(ggplot2::aes(colour = color), size = pt.size, shape = 21, alpha = 0.6, show.legend = FALSE)
  }
  p <- p + ggplot2::xlim(c(xmin, xmax)) + ggplot2::ylim(c(ymin, ymax)) +
       ggplot2::scale_color_manual(values = cols) +
       ggplot2::scale_fill_manual(values = cols) +
       ggplot2::geom_hline(yintercept = -log10(padj), linetype = "dotted") +
       ggplot2::geom_vline(xintercept = c(-logFC, logFC), linetype = "dotted") +
  if (isTRUE(box)){
    p <- ggrepel::geom_label_repel(ggplot2::aes(label=label,color=group), show.legend = FALSE, fontface = "bold", size = label.size,
                                   box.padding = ggplot2::unit(0.8, "lines"), point.padding = ggplot2::unit(0.3, "lines"), segment.size = 0.3)
  }else{
    p <- ggrepel::geom_text_repel(ggplot2::aes(label=label,color=group), show.legend = FALSE, fontface = "bold", size = label.size,
                                   box.padding = ggplot2::unit(0.8, "lines"), point.padding = ggplot2::unit(0.3, "lines"), segment.size = 0.3)
  }
  p <- p + ggplot2::labs(x = "Log2 Fold change", y = "Log10 P-adjust", title = paste0(key, " in cluster-", cluster)) + cowplot::theme_cowplot()
  return(p)
}
