...
#' @author rstatistics
#' @export
scDGE <- function(object = object, comparison = c("condA", "condB"), group.by = NULL,
    min.cells = 20, logFC = 0.25, clusters = NULL) {
    results <- list()
    ...
    for (cluster in clusters) {
        cat(paste0("### ", "Comparing cluster-", cluster, " between ", condA, " and ",
            condB, " ...\n"))
        ...
        if (all(comparison %in% names(table(Object[[group.by]])))) {
            ...
            warning("Cluster ", cluster, " has no cell in ", condA, " or ", condB,
                ". Ignored.\n")
            next
        }
        if (!all(as.data.frame(table(Object[[group.by]]))$Freq >= min.cells)) {
            warning("Cell number in cluster ", cluster, " is less than ", min.cells,
                " cells. Ignored.\n")
            next
        ...
        tmpDE <- suppressWarnings(Seurat::FindMarkers(object = Object, assay = "RNA",
            ident.1 = condA, ident.2 = condB, group.by = group.by, min.cells.group = min.cells,
