...
scIntegrate <- function(object.list = NULL, object.names = NULL, nVariable = 2000,
    nPC = 30, res = 0.5, batch.rm = "harmony") {
    ...
    object <- merge(object.list[[1]], y = object.list[2:length(object.list)], add.cell.ids = as.character(object.names),
        project = "seurat")
    ...
    object <- Seurat::ScaleData(object, vars.to.regress = c("nCount_RNA", "percent.mito"),
        verbose = TRUE)
    object <- Seurat::RunPCA(object, pc.genes = Seurat::VariableFeatures(object),
        npcs = nPC, verbose = TRUE)

    options(repr.plot.height = 5, repr.plot.width = 12)
    p1 <- Seurat::DimPlot(object = object, reduction = "pca", pt.size = 0.1, group.by = "sample")
    p2 <- Seurat::VlnPlot(object = object, features = "PC_1", pt.size = 0.1, group.by = "sample")
    plot(cowplot::plot_grid(p1, p2))
    object <- harmony::RunHarmony(object = object, group.by.vars = "sample", assay.use = Seurat::DefaultAssay(object),
    ...
    if (batch.rm == "harmony") {
        ...
        object <- Seurat::RunTSNE(object, reduction = "harmony", dims = seq_len(nPC),
            do.fast = TRUE)
