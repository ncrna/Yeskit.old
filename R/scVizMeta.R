...
scVizMeta <- function(object=NULL, signature=NULL, pt.size=NULL, reduction=NULL, title=NULL, split.by=NULL, ncol=NULL, raster=TRUE, palette="YlOrRd",
                      interval=c(Max=100000, Hyper=10000, Abundant=1000, Large=500, Medium=100, Small=10, Single=1, None=0)){
  ...
  for (x in seq_along(interval)){
      names(interval)[x] <- paste0(names(interval[x]), ' (', interval[x+1], ' < X <= ', interval[x], ')')
  }
  ...
  ps <- function(data, cols, title=NULL, legend_title=NULL){
    ...
    if (isTRUE(raster)){
      p <- p + ggrastr::rasterise(dpi = 300, ggplot2::geom_point(ggplot2::aes(colour = class), size = pt.size, na.rm = TRUE))
    }else{
      p <- p + ggplot2::geom_point(ggplot2::aes(colour = class), size = pt.size, na.rm = TRUE)
    }
    p <- p + cowplot::theme_cowplot() + ggplot2::guides(colour=ggplot2::guide_legend(override.aes=list(size=5))) +
             ggplot2::scale_color_manual(values=cols) + ggplot2::labs(x=colnames(data)[1], y=colnames(data)[2], title=title)
  ...
  pm <- function(data, cols=cols, min.value=min.value, max.value=max.value, title=NULL, legend_title=NULL){
    p <- ggplot2::ggplot(data=data, ggplot2::aes(x=data[,1], y=data[,2]))
    if (isTRUE(raster)){
      p <- p + ggrastr::rasterise(dpi = 300, ggplot2::geom_point(ggplot2::aes(colour = class), size = pt.size, na.rm = TRUE))
    }else{
