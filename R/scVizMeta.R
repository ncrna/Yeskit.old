...
scVizMeta <- function(object = NULL, signature = NULL, pt.size = NULL, reduction = NULL,
    title = NULL, split.by = NULL, ncol = NULL, raster = TRUE, palette = "YlOrRd",
    ...
    for (x in seq_along(interval)) {
        names(interval)[x] <- paste0(names(interval[x]), " (", interval[x + 1], " < X <= ",
            interval[x], ")")
    }
    ...
    ps <- function(data, cols, title = NULL, legend_title = NULL) {
        p <- ggplot2::ggplot(data = data, ggplot2::aes(x = data[, 1], y = data[,
            2]))
        if (isTRUE(raster)) {
            p <- p + ggrastr::rasterise(dpi = 300, ggplot2::geom_point(ggplot2::aes(colour = class),
                size = pt.size, na.rm = TRUE))
        } else {
            p <- p + ggplot2::geom_point(ggplot2::aes(colour = class), size = pt.size,
                na.rm = TRUE)
        }
        p <- p + cowplot::theme_cowplot() + ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 5))) +
            ggplot2::scale_color_manual(values = cols) + ggplot2::labs(x = colnames(data)[1],
            y = colnames(data)[2], title = title)
        p <- p + ggplot2::scale_x_continuous(limits = c(xmin, xmax), breaks = seq(floor(xmin/5) *
            5, ceiling(xmax/5) * 5, by = 5)) + ggplot2::scale_y_continuous(limits = c(ymin,
            ymax), breaks = seq(floor(ymin/5) * 5, ceiling(ymax/5) * 5, by = 5))
        return(p)
    }
    pm <- function(data, cols = cols, min.value = min.value, max.value = max.value,
    ...
    if (is.null(x = split.by)) {
        ...
        for (i in seq_len(length(interval) - 1)) {
            Data$class <- ifelse(Data[, signature] > interval[i + 1] & Data[, signature] <=
                interval[i], names(interval[i]), Data$class)
        }
        ...
        ymin <- min(Data[, 2])
        ymax <- max(Data[, 2])
        cols <- c(rev(RColorBrewer::brewer.pal(length(levels(Data$class)) - 1, palette))[1:length(levels(Data$class)) -
            1], "gray")
        # names(cols) <- names(interval)
        names(cols) <- names(interval)[(length(interval) - length(levels(Data$class)) +
            1):length(interval)]
    ...
    return((patchwork::wrap_plots(plots, ncol = ncol) | legend) + patchwork::plot_layout(widths = c(3,
        1)))
}
