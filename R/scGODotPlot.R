...
scGODotPlot <- function(object = NULL, key = NULL, clusters = NULL, direction = "up",
    ont = "BP", padj.cutoff = 0.01, order.genes = FALSE, colors = NULL, font.size = 8,
    top_n = 5, extra = NULL) {
    if (is.null(object)) {
    ...
    GO.data <- data.frame(Cluster = character(), Term = character(), GeneNum = character(),
        Value = numeric())
    for (i in clusters) {
        ...
        for (GO.term in GO.terms) {
            if (nrow(subset(z, Term == GO.term)) == 0) {
                ...
                GO.value = -log10(as.numeric(gsub("< ", "", subset(z, Term == GO.term)[1,
                  ]$Fisher.elim)))
    ...
    p <- ggplot2::ggplot(GO.data, ggplot2::aes(x = Cluster, y = Term)) + ggplot2::geom_point(ggplot2::aes(colour = Value,
        size = GeneNum), stat = "identity") + ggplot2::theme(plot.title = ggplot2::element_text(face = "bold",
        vjust = 1), axis.title.x = ggplot2::element_text(face = "bold", vjust = -0.2),
        axis.title.y = ggplot2::element_text(face = "bold"), axis.text.y = ggplot2::element_text(hjust = 1,
            colour = "black"), axis.text.x = ggplot2::element_text(angle = -45, vjust = 1,
            hjust = 0, colour = "black"), panel.background = ggplot2::element_blank(),
        axis.ticks = ggplot2::element_blank(), axis.text = ggplot2::element_text(size = font.size),
        panel.border = ggplot2::element_rect(colour = "black", fill = NA, size = 1),
        panel.grid.minor = ggplot2::element_line(colour = "grey", linetype = "dotted",
            size = 0.1), panel.grid.major = ggplot2::element_line(colour = "grey",
            linetype = "dotted", size = 0.1)) + ggplot2::scale_colour_gradient(low = colors[1],
        high = colors[2], na.value = "white", breaks = breaks, labels = as.character(breaks),
        limits = c(-log10(padj.cutoff), max(breaks))) + ggplot2::labs(x = "Cluster",
        y = "GO Terms", color = "-Log10(P.adjust)") + ggplot2::scale_y_discrete(limits = rev(levels(GO.data$Term)))
    if (is.null(extra)) {
        p <- p + ggplot2::ggtitle(paste0(ifelse(direction == "up", "Up", "Down"),
            "-regulated GO ", ifelse(ont == "BP", "Biological Process", ifelse(ont ==
                "CC", "Cellular Component", "Molecular Function"))))
    } else {
        p <- p + ggplot2::ggtitle(paste0(ifelse(direction == "up", "Up", "Down"),
            "-regulated GO ", ifelse(ont == "BP", "Biological Process", ifelse(ont ==
                "CC", "Cellular Component", "Molecular Function")), " (", extra,
            ")"))
    }
    return(p)
}
