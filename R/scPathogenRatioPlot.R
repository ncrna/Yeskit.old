...
scPathogenRatioPlot <- function(object=NULL, species=NULL, cols=NULL, split.by=NULL, ncol=NULL){
  options(scipen=5)
  if (is.null(cols)){
    cols <- c("gray80", "red")
  }
  ...
  pp <- function(data=NULL, title=NULL, Legend=FALSE){
    data.Positive <- data.frame(colSums(table(data[data[, species]>0,])), Status="Positive")
    if (nrow(data[data[, species]>0,])==0){
      data.Positive <- data.frame(colSums(table(data[data[, species]>0,])), Counts=0, Status="Positive")
    }
    data.Positive[, "Cluster"] <- factor(rownames(data.Positive), levels = as.character(rownames(data.Positive)))
    colnames(data.Positive) <- c("Frequency", "Status", "Cluster")
    data.Negative <- data.frame(colSums(table(data[data[, species]==0,])), Status="Negative")
    if (nrow(data[data[, species]==0,])==0){
      data.Negative <- data.frame(colSums(table(data[data[, species]==0,])), Counts=0, Status="Negative")
    }
    data.Negative[, "Cluster"] <- factor(rownames(data.Negative), levels = as.character(rownames(data.Negative)))
    colnames(data.Negative) <- c("Frequency", "Status", "Cluster")
    data <- rbind(data.Positive, data.Negative)
    data[, "Status"] <- factor(data[, "Status"], levels = c("Negative", "Positive"))
    p <- ggplot2::ggplot(data, ggplot2::aes(x=Cluster, y=Frequency, fill=Status)) +
  ...
  if (! is.null(x = split.by)){
    ...
    legend <- pp(object@meta.data[, c(species, "seurat_clusters")], title = NULL, Legend = TRUE)
