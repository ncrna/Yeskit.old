...
scPathogenRatioPlot <- function(object = NULL, species = NULL, cols = NULL, split.by = NULL,
    ncol = NULL) {
    options(scipen = 5)
    if (is.null(cols)) {
        cols <- c("gray80", "red")
    ...
    pp <- function(data = NULL, title = NULL, Legend = FALSE) {
        ...
        p <- ggplot2::ggplot(data, ggplot2::aes(x = Cluster, y = Frequency, fill = Status)) +
            ggplot2::geom_bar(position = "fill", stat = "identity") + cowplot::theme_cowplot() +
            ggplot2::scale_fill_manual(values = cols) + ggplot2::scale_x_discrete(expand = c(0.05,
            0.05)) + ggplot2::scale_y_continuous(expand = c(0.01, 0.01), labels = c(0,
            25, 50, 75, 100)) + ggplot2::labs(x = "Clusters", y = "Fraction of cells (%)",
            title = title)
        if (isFALSE(Legend)) {
