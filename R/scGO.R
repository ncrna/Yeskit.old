...
scGO <- function(object=NULL, key=NULL, clusters=NULL, logFC = 0.25, only.pos=TRUE, reference="human", extra=NULL){
  ...
  if (reference == "human"){
	...
	for (c in as.character(unique(markers$cluster))){
	  ...
	  for (direction in directions){
	    if (direction == "up"){
	      queryMatrix = subset(markers[order(markers$pct.1 / markers$pct.2, decreasing = TRUE),], cluster==eval(quote(c)) & avg_log2FC > abs(logFC) & p_val_adj < 1e-2)
	    }else{
	      queryMatrix = subset(markers[order(markers$pct.2 / markers$pct.1, decreasing = TRUE),], cluster==eval(quote(c)) & avg_log2FC < -abs(logFC) & p_val_adj < 1e-2)
	    }
