...
#' @param only.pos Only up-regulated genes are taken into account, default only.pos=TRUE
#' @param reference Reference organism to set ["human" or "mouse"], default reference = "human"
#' @return Table that stores scPathogenGO results
#'
#' @author Wei Zhang
#' @export

scPathogenGO <- function(object=NULL, key=NULL, species=NULL, clusters=NULL, logFC = 0.25, only.pos=TRUE, reference="human"){
  ...
  for (C in clusters){
    ...
    print(paste0("Running ", species, " in cluster-", C, " ... "))

    for (direction in c("up", "down")){
      if (direction == "up"){
        queryMatrix = subset(DEdata[order(DEdata$pct.1 / DEdata$pct.2, decreasing = TRUE),], avg_log2FC > abs(logFC) & p_val_adj < 1e-2)
      }else{
        queryMatrix = subset(DEdata[order(DEdata$pct.2 / DEdata$pct.1, decreasing = TRUE),], avg_log2FC < -abs(logFC) & p_val_adj < 1e-2)
      }
      ...
      for (ont in c("BP", "CC", "MF")){
        GOdata <- suppressMessages(new("topGOdata", ontology = ont, allGenes = as.factor(queryGeneList),
                                       nodeSize = 10, annot = topGO::annFUN.org, mapping = mappings, ID = "entrez"))
        resultTopGO.elim <- suppressMessages(topGO::runTest(GOdata, algorithm = "elim", statistic = "Fisher"))
        tab[[ont]] <- rbind(tab[[ont]], topGO::GenTable(GOdata, Fisher.elim = resultTopGO.elim,
