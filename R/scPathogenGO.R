...
scPathogenGO <- function(object = NULL, key = NULL, species = NULL, clusters = NULL,
    logFC = 0.25, only.pos = TRUE, reference = "human") {
    ...
    for (C in clusters) {
        ...
        if (nrow(DEdata) == 0) {
            warning("No DE genes between species ", species, " pos/neg in cluster-",
                C)
            next
        }
        ...
        for (direction in c("up", "down")) {
            if (direction == "up") {
                queryMatrix = subset(DEdata[order(DEdata$pct.1/DEdata$pct.2, decreasing = TRUE),
                  ], avg_log2FC > abs(logFC) & p_val_adj < 0.01)
            } else {
                queryMatrix = subset(DEdata[order(DEdata$pct.2/DEdata$pct.1, decreasing = TRUE),
                  ], avg_log2FC < -abs(logFC) & p_val_adj < 0.01)
            }
            ...
            for (ont in c("BP", "CC", "MF")) {
                ...
                resultTopGO.elim <- suppressMessages(topGO::runTest(GOdata, algorithm = "elim",
                  statistic = "Fisher"))
