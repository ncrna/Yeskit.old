...
scGOBarPlot <- function(object = NULL, key = NULL, cluster = NULL, direction = "up",
    ont = "BP", top_n = 20, extra = NULL) {
    Term <- FDR <- NULL
    if (is.null(object)) {
    ...
    p <- ggplot2::ggplot(z, ggplot2::aes(x = reorder(Term, -log10(FDR)), y = -log10(FDR))) +
        ...
        axis.line.x = ggplot2::element_line(colour = "black"), axis.line.y = ggplot2::element_line(colour = "black"),
        axis.ticks.y = ggplot2::element_blank()) + ggplot2::scale_y_continuous(expand = c(0,
        0.015)) + ggplot2::labs(x = "GO Terms", y = "-Log10(P.adjust)")
    if (is.null(extra)) {
