...
scGOBarPlot <- function(object=NULL, key=NULL, cluster=NULL, direction="up", ont="BP", top_n=20, extra=NULL){
  Term <- FDR <- NULL
  if (is.null(object)){
  ...
  p <- ggplot2::ggplot(z, ggplot2::aes(x=reorder(Term,-log10(FDR)),y=-log10(FDR))) +
    ...
    ggplot2::geom_hline(yintercept = 2, linetype = "dashed", size = 0.5, color = "grey50") +
    ggplot2::coord_flip() + ggplot2::theme(panel.grid.major = ggplot2::element_blank(),
                         panel.grid.minor = ggplot2::element_blank(),
                         panel.background = ggplot2::element_blank(),
                         axis.title = ggplot2::element_text(family = "ArialMT", size = 12, colour = "black"),
                         axis.text = ggplot2::element_text(family = "ArialMT", colour = "black"),
                         axis.line.x = ggplot2::element_line(colour = "black"),
                         axis.line.y = ggplot2::element_line(colour = "black"),
                         axis.ticks.y = ggplot2::element_blank()) +
    ggplot2::scale_y_continuous(expand = c(0, 0.015)) +
    ggplot2::labs(x = "GO Terms", y = "-Log10(P.adjust)")
  if (is.null(extra)){
    p <- p + ggplot2::ggtitle(paste0(ifelse(direction=="up", "Up", "Down"), "-regulated GO ", ont, " in cluster-", cluster))
  }else{
    p <- p + ggplot2::ggtitle(paste0(ifelse(direction=="up", "Up", "Down"), "-regulated GO ", ont, " in cluster-", cluster, " (", extra, ")"))
  }
  return(p)
}
