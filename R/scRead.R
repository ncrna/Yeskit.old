...
#' @param human.prefix The prefix denoting rownames for human cells. Default is 'GRCh38_'. Just for PDX samples
#' @param mouse.prefix The prefix denoting rownames for mouse cells. Default is 'mm10___'. Just for PDX samples
#' @param organism.thres Threshould fraction of reads to separate organism and background, Default is 0.9. Just for PDX samples
#' @param organism.use Which organism is used. Default is NULL, means all organisms will be used. Just for PDX samples
...
scRead <- function(sample_name = NULL, data_dir = NULL, gene_column = 2, min_cells = 5,
    min_features = 200, min_rnas = 1000, percent_mito = 20, mito_pattern = "^GRCh38_MT-|^mm10___mt-|^MT-|^mt-",
    project_name = "Seurat", group_name = NULL, strip_suffix = TRUE, meta_file = NULL,
    human.prefix = "GRCh38_", mouse.prefix = "mm10___", organism.thres = 0.9, organism.use = NULL,
    meta_features = NULL) {
    object <- Seurat::Read10X(data.dir = data_dir, gene.column = gene_column, strip.suffix = strip_suffix)
    hsaFeatures <- grep(pattern = human.prefix, x = rownames(x = object), value = TRUE)
    mmuFeatures <- grep(pattern = mouse.prefix, x = rownames(x = object), value = TRUE)
    ...
    if (length(hsaFeatures) > 0 | length(mmuFeatures) > 0) {
        ...
        organism.info <- data.frame(organism = c(rep("human", length(colnames(hsa.object))),
            rep("mouse", length(colnames(mmu.object)))), row.names = c(colnames(hsa.object),
            colnames(mmu.object)))
        if (is.null(organism.use)) {
            indcs <- which(colnames(object) %in% c(colnames(hsa.object), colnames(mmu.object)))
    ...
    if (length(organism.info) > 0) {
        organism_df <- merge(data.frame(object$organism, row.names = colnames(object)),
            organism.info, by = 0, order = FALSE)
        object$organism <- organism_df[, "organism"]
    }
    mito_pattern <- gsub("_", "-", mito_pattern)
    object[["percent.mito"]] <- Seurat::PercentageFeatureSet(object, pattern = mito_pattern)
    plot(Seurat::VlnPlot(object = object, features = c("nFeature_RNA", "nCount_RNA",
        "percent.mito"), ncol = 3, pt.size = 0.01))

    if (!is.null(meta_file)) {
        if (file.exists(meta_file)) {
            metadata <- read.table(meta_file, header = TRUE, row.names = 1, stringsAsFactors = FALSE,
    ...
    object <- subset(object, subset = nFeature_RNA > max(min_features, quantile(object$nFeature_RNA,
        0.01)) & nFeature_RNA < quantile(object$nFeature_RNA, 0.99) & percent.mito <=
        min(percent_mito, quantile(object$percent.mito, 0.99)) & nCount_RNA > max(min_rnas,
        quantile(object$nCount_RNA, 0.01)) & nCount_RNA < quantile(object$nCount_RNA,
        0.99))
    plot(Seurat::VlnPlot(object = object, features = c("nFeature_RNA", "nCount_RNA",
        "percent.mito"), ncol = 3, pt.size = 0.01))
    return(object)
