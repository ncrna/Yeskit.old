...
scRead <- function(sample_name=NULL, data_dir=NULL, gene_column=2, min_cells=5, min_features=200, percent_mito=20,
  ...
  if (length(hsaFeatures)>0 | length(mmuFeatures)>0){
    ...
    organism.info <- data.frame(organism=c(rep("human", length(colnames(hsa.object))), rep("mouse", length(colnames(mmu.object)))),
                               row.names = c(colnames(hsa.object), colnames(mmu.object)))
  ...
  if (!is.null(meta_features)){
    ...
    object <- Seurat::CreateSeuratObject(counts = object[!rownames(object) %in% meta_features, ], min.cells = min_cells,
                                         min.features = min_features, project = project_name, meta.data = metadata)
  }else{
  ...
  if (length(organism.info)>0){
    organism_df <- merge(data.frame(object$organism, row.names = colnames(object)), organism.info, by = 0, order=FALSE)
  ...
  if (!is.null(meta_file)){
    if (file.exists(meta_file)){
      ...
      if (nrow(metadata)>0){
        object <- Seurat::AddMetaData(object = object, metadata = t(metadata), col.name = metadata_ids)
      }
    }else{
