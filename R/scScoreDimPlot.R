...
scScoreDimPlot <- function(object=NULL, signature=NULL, reduction=NULL, cols=NULL, pt.size=NULL, split.by=NULL, title=NULL, ncol=NULL, raster=TRUE, scale=TRUE, col.min=NA, col.max=NA){
  ...
  if (is.null(x = split.by)){
    ...
    ymin <- min(Data[,2])
    ymax <- max(Data[,2])
    return(ps(data=Data, min.value = col.min, max.value = col.max, title=title, legend_title = signature))
  }
  plots <- list()
  if (! is.null(x = split.by)){
    ...
    legend <- ps(data=Data, min.value = col.min, max.value = col.max, title=NULL, legend_title=signature)
