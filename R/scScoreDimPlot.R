...
scScoreDimPlot <- function(object = NULL, signature = NULL, reduction = NULL, cols = NULL,
    pt.size = NULL, split.by = NULL, title = NULL, ncol = NULL, raster = TRUE, scale = TRUE,
    col.min = NA, col.max = NA) {
    ...
    ps <- function(data, min.value = min.value, max.value = max.value, title = NULL,
        legend_title = NULL) {
        data <- suppressWarnings(data.frame(apply(data, 2, as.numeric)))
        p <- ggplot2::ggplot(data = data, ggplot2::aes(x = data[, 1], y = data[,
            2], z = data[, 3]))
        if (isTRUE(raster)) {
            p <- p + ggrastr::rasterise(dpi = 300, ggplot2::geom_point(ggplot2::aes(colour = data[,
                3]), size = pt.size, na.rm = TRUE))
        } else {
        ...
        p <- p + ggplot2::scale_colour_gradientn(colours = cols, na.value = "white",
            breaks = seq(min.value, max.value, length = 5), limit = c(min.value,
                ...
                order = 1, title.theme = ggplot2::element_text(size = 8), label.theme = ggplot2::element_text(size = 8)))
    ...
    pm <- function(data, min.value = min.value, max.value = max.value, title = NULL,
        legend_title = NULL) {
        data <- suppressWarnings(data.frame(apply(data, 2, as.numeric)))
        p <- ggplot2::ggplot(data = data, ggplot2::aes(x = data[, 1], y = data[,
            2], z = data[, 3]))
    ...
    if (is.null(x = split.by)) {
        ...
        ymin <- min(Data[, 2])
        ymax <- max(Data[, 2])
        return(ps(data = Data, min.value = col.min, max.value = col.max, title = title,
    ...
    if (!is.null(x = split.by)) {
        ...
        legend <- ps(data = Data, min.value = col.min, max.value = col.max, title = NULL,
            legend_title = signature)
