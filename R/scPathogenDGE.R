...
#' @author Wei Zhang
#' @export
scPathogenDGE <- function(object = NULL, species.by = NULL, clusters = NULL, min.cells = 20){
  if (is.null(object)){
  ...
  for (feature in species.by){
    ...
    if (is.null(results[[feature]])){ results[[feature]] = list() }
    ...
    for (cluster in clusters){
      ...
      tmpDE <- suppressMessages(suppressWarnings(Seurat::FindMarkers(object = Object, assay = 'RNA', ident.1 = "Pos", ident.2 = "Neg", group.by = "group",
  ...
  # clean up results
  for (feature in species.by){
    if (length(results[[feature]]) == 0){ results[[feature]] <- NULL; next }
    for (cluster in clusters){
      if (length(results[[feature]][[cluster]]) == 0){ next }
      results[[feature]][[cluster]] <- subset(results[[feature]][[cluster]], p_val_adj <= 1)
      if (nrow(results[[feature]][[cluster]]) <= 1){ results[[feature]][[cluster]] <- NULL; next }
    }
    if (length(results[[feature]]) == 0){ results[[feature]] <- NULL; next }
  }
  return(results)
